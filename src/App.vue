<template>
    <a-config-provider :locale="locale">
        <a-layout style="min-height: 100vh">
            ...
            <a-layout-sider ref="sider" v-model:collapsed="store.collapsed" :trigger="null" collapsible
                :style="{ overflow: 'auto', minHeight: '100vh', position: 'fixed', left: 0, top: 0, bottom: 0 }">

                <div style="margin: 10px 0 6px 0;text-align: center;">
                    <img src="/electron.png" height="48">
                </div>

                <a-menu theme="dark" mode="inline" @click="menuClick" v-model:selectedKeys="selectedKeys"
            ...
            <a-layout :style="{ marginLeft: store.collapsed ? '80px' : '200px', transition: 'margin-left 0.2s' }">
                <a-layout-header ref="header" :style="{ background: '#fff', padding: '0' }">

                    <a-row type="flex">
                        <a-col flex="50px">
                            <menu-unfold-outlined v-if="store.collapsed" class="trigger" @click="menuUnfold" />

                            <menu-fold-outlined v-else class="trigger" @click="() => store.changeMenuFoldState('fold')" />
                        </a-col>
                        ...
                        <a-col flex="0 1 134px">
                            <a-tooltip placement="leftTop">
                                ...
                                <a-select style="width: 110px;" v-show="store.showTokenType"
                                    v-model:value="selectTokenValue" :options="tokenOptions"
                        ...
                        <a-col flex="0 1 134px">
                            <a-tooltip placement="left">
                                ...
                                <a-select style="width: 110px;" v-show="store.showMainnet" v-model:value="selectValue"
                                    :options="options" @change="handleMainnetChange" />
                            </a-tooltip>
                        </a-col>
                    </a-row>
                    ...
                    <a-modal v-model:open="visible" title="自定义主网" style="top: 180px" @cancel="handleModalCancel">
                        ...
                        <template #footer>
                            ...
                            <a-button type="danger" ghost @click="handleModalCancel">取消</a-button>
                        </template>
                    </a-modal>
                </a-layout-header>
...
export default defineComponent({
    ...
    mounted() {
        let height = this.$refs.sider.$el.offsetHeight - this.$refs.header.$el.offsetHeight
        height -= 48
        this.height = height
    },
    methods: {
        ...
        getCurrentGasPrice() {
            this.store.web3.eth.getGasPrice((error, gasPrice) => {
                if (error === null) {
                    gasPrice = this.store.web3.utils.fromWei(gasPrice, 'Gwei')
                    gasPrice = Math.ceil(parseFloat(gasPrice) + parseFloat(this.store.maxPriorityFeePerGas)) + ''
