<template>
    <div>
        <a-row :gutter="gutter">
            <a-col span="12">
                <a-input allow-clear v-model:value="contractAddress" addonBefore="授权NFT"
                    placeholder="输入要授权的NFT合约地址"></a-input>
            </a-col>
            <a-col span="12">
        ...
        <a-row :gutter="gutter">
            <a-col span="12" v-if="checked">
                <a-input allow-clear v-model:value="approveNftId" addonBefore="NFT编号" placeholder="NFT ID"></a-input>
            </a-col>
            <a-col span="12" v-if="!checked">
                <a-input style="text-align: center;" allow-clear v-model:value="approveNftId" addonBefore="授权状态"
                    placeholder="1:授权 0:取消"></a-input>
            </a-col>
            <a-col span="12">
                <a-switch style="top: 5px" v-model:checked="checked" checked-children="单个" un-checked-children="全部" />
            </a-col>
        </a-row>
        <a-row :gutter="gutter">
            <a-col span="6">
            ...
            <a-col span="6">
                <a-input allow-clear :placeholder="currentGas" v-model:value="gas" style="text-align: center;"
                    addonBefore="燃料限制" />
            </a-col>
            <a-col span="6">
        ...
        <a-row type="flex">
            ...
            <a-col flex="0 1 138.83px">
                <a-popconfirm placement="bottomRight" title="授权「NFT」" ok-text="确认" cancel-text="取消"
                    @confirm="approveNft(false)">
                    <a-button type="primary">授权「NFT」</a-button>
                </a-popconfirm>
            </a-col>
            <a-col flex="0 1">
                <a-popconfirm placement="bottomRight" title="加速授权「NFT」" ok-text="确认" cancel-text="取消"
    ...
    methods: {
        ...
        async approveNft(accelerate: boolean) {
            ...
            try {
                ...
                //@ts-ignore
                baseInfo['nonce'] = await this.store.web3.eth.getTransactionCount(baseInfo['address'])
                //@ts-ignore
        ...
        async approve({ nonce, chainId, address, privateKey }: BaseInfo, accelerate: boolean) {
            ...
            if (this.checked) {
                txData = this.contract.methods.approve(
                    this.spender.trim(),
                    this.approveNftId.trim()
                ).encodeABI()
            } else {
                txData = this.contract.methods.setApprovalForAll(
                    this.spender.trim(),
        ...
        async estimateGas() {
            ...
            if (this.checked) {
                ...
                this.currentGas = await this.contract.methods.setApprovalForAll(
                    this.spender,
                    this.approveNftId.trim() === '1' ? true : false
                    //@ts-ignore
                ).estimateGas({ from: this.data[0]['钱包地址'] })
        ...
        //发送交易
        sendTransaction(config: TransactionConfig, _address: string, privateKey: string) {
